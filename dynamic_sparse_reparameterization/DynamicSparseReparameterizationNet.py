...
class DynamicSparseReparameterizationLayer(nn.Module):
    ...
    def prune(self, H) -> int:
        k = torch.mul((self.weight.abs() < H), (self.weight != 0)).count_nonzero()
        with torch.no_grad():
...
class DynamicSparseReparameterizationNet(nn.Module):
    def __init__(self, layers, H, sparsity, Np, fractional_tolerence, verbose=False) -> None:
        ...
        for l in range(len(layers[:-1])):
            self.layers.append(DynamicSparseReparameterizationLayer(n_input=layers[l], n_output=layers[l+1], sparsity=sparsity))
